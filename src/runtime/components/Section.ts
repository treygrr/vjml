...
const metadata = requireVjmlComponentMetadata('mj-section')
...
function buildBackgroundStyle(
  ...
  const hasBackground = hasNonEmptyAttribute(attrs['background-url'])
...
function getSectionChildOutlookStyle(
  childAttrs: Readonly<Record<string, string>>,
  childTagName: string | null,
...
export default createVjmlComponent(metadata, {
  ...
  render({ activeMjClass, attrs, content, documentContext }, extra) {
    ...
    const wrappedChildren = [
      createVjmlStaticHtml(conditionalTag('<tr>')),
        ...
        return [
          createVjmlStaticHtml(conditionalTag(
            `<td${renderHtmlAttributes({
              ...
              style: getSectionChildOutlookStyle(
                childAttrs,
                entry.tagName,
                `${box}px`,
    ...
    if (fullWidth) {
      return h('table', {
        ...
        h('tbody', [
          h('tr', [
            h('td', [before, sectionContent, after]),
          ]),
        ]),
      ])
    }
